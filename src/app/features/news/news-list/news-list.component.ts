...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, RouterLink, FormsModule, PageLayoutComponent, PaginationComponent],
  template: `
    <app-page-layout>
        ...
        @if (loading()) {
          ...
          <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-px" style="background:rgba(255,255,255,0.05)">
            @for (item of news(); track item.id) {
              <article class="group bg-ink hover:bg-[#1a0000] transition-colors p-6 flex flex-col">
                @if (item.imageUrl) {
                ...
                <div class="flex items-center justify-between mt-auto">
                  ...
                  <a [routerLink]="['/news', item.id]"
                    class="w-9 h-9 rounded-full border border-white/20 flex items-center justify-center hover:bg-accent hover:border-accent transition-colors text-sm">→</a>
                </div>
              </article>
            }
          </div>
