...
@Component({
  ...
  template: `
    <app-admin-layout>
      <div>
        <a routerLink="/admin/licences" class="inline-flex items-center gap-2 text-white/50 hover:text-white text-sm mb-8 transition-colors">← Retour</a>
        <h1 class="font-serif text-3xl mb-8">{{ isEdit ? 'Modifier la licence' : 'Nouvelle licence' }}</h1>
        ...
        <form (ngSubmit)="submit()" class="grid grid-cols-2 gap-x-10 gap-y-8 max-w-2xl">
          ...
          <div class="col-span-2 flex gap-4 pt-4">
            ...
            <a routerLink="/admin/licences" class="px-8 py-3 rounded-full border border-white/20 hover:border-white text-sm transition-colors">Annuler</a>
          </div>
        </form>
      </div>
    </app-admin-layout>
...
export class LicenceFormComponent implements OnInit {
  ...
  submit(): void {
    ...
    obs.subscribe({
      next: () => this.router.navigate(['/admin/licences']),
