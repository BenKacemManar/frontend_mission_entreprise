...
@Component({
  ...
  template: `
    <app-admin-layout>
      <div>
        <a routerLink="/admin/news" class="inline-flex items-center gap-2 text-white/50 hover:text-white text-sm mb-8 transition-colors">← Retour</a>
        <h1 class="font-serif text-3xl mb-8">{{ isEdit ? "Modifier l'article" : 'Nouvel article' }}</h1>
        ...
        <form (ngSubmit)="submit()" class="space-y-8 max-w-2xl">
          ...
          <div class="flex gap-4">
            ...
            <a routerLink="/admin/news" class="px-8 py-3 rounded-full border border-white/20 hover:border-white text-sm transition-colors">Annuler</a>
          </div>
        </form>
      </div>
    </app-admin-layout>
...
export class NewsFormComponent implements OnInit {
  ...
  readonly CATS = ['COMPETITION','FORMATION','GOUVERNANCE','INFRASTRUCTURE','GENERAL'];
